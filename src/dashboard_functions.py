...
pit_stops = (
    pit_stops.groupby(["raceId", "driverId"]).sum().drop(columns=["stop", "lap"])
)
...
podium_data = pd.read_html(
    "https://en.everybodywiki.com/List_of_Formula_One_podium_finishers"
)
...
def driver_data():
    list_of_formula_one_drivers = pd.read_html(
    ...
    name_nationality = name_nationality.drop(
        "driver_name", 1
    )  # droppping diver name colunm leaving us just with nationality and a driver unit(1 driver name = 1 driver)
    ...
    def update_data(value_, new_value):
        ...
        name_nationality.iloc[
            index, name_nationality.columns.get_loc("Nationality")
        ] = new_value
        return index
    ...
    name_nationality = name_nationality.drop(
        [name_nationality.index[865], name_nationality.index[505]]
    )
    ...
    name_nationality = name_nationality.drop(
        name_nationality.index[0]
    )  # weird 1 in the countries column with a 0 for count, deleting
...
def top_driver_points_gauge():
    ...
    gname = data["MRData"]["StandingsTable"]["StandingsLists"][0]["DriverStandings"][0][
        "Driver"
    ]["givenName"]
    fname = data["MRData"]["StandingsTable"]["StandingsLists"][0]["DriverStandings"][0][
        "Driver"
    ]["familyName"]
    max_points = 23 * 25
...
def time_to_next_race_gauge():
    ...
    pct_ttnr = round(
        (((now - previous_race_val) / (next_race_val - previous_race_val))) * 100, 2
    )
...
def pit_pos_tracks():
    data_by_tracks = (
        pit_data.groupby(["name", "position"])
...
def podium_country():
    ...
    by_country = (
        podium_data.loc[:, ["Country", "Podiums"]]
        .groupby("Country")
        .sum()
        .reset_index()
        .sort_values("Podiums", ascending=True)
    )
    by_country = by_country.sort_values(by="Podiums", ascending=False).nlargest(
        10, "Podiums"
    )
...
def podium_driver():
    ...
    by_driver = (
        podium_data.loc[:, ["Driver", "Podiums"]]
        .groupby("Driver")
        .sum()
        .reset_index()
        .sort_values("Podiums", ascending=True)
    )
    by_driver = by_driver.sort_values(by="Podiums", ascending=False).nlargest(
        20, "Podiums"
    )
...
def constructor_sunburst():
    plot_constructor = px.sunburst(
        sunburst_data,
        path=["Year", "City", "Winning constructor"],
        width=1000,
        height=800,
...
def driver_sunburst():
    plot_driver = px.sunburst(
        ...
        color_discrete_map={
            "Lewis Hamilton": "black",
            "Sebastian Vettel": "gold",
            "Lewis Hamilton": "darkblue",
        },
...
def drivers_per_country():
    ...
    driver_data.plot.bar(
        xlabel="Country",
        ylabel="Amount of Drivers",
        title="Drivers Per Country 1950-2022",
        figsize=(12, 10),
        ax=ax,
    )
...
def tech_improvement_data():
    ...
    for i in range(2009, 2020):
        ...
        # Select fact
        qualiDataAUS = response_dataAUS["MRData"]["RaceTable"]["Races"][0][
            "QualifyingResults"
        ][0]["Q3"]
        dicAUS[f"{i}"] = qualiDataAUS
        qualiDataMON = response_dataMON["MRData"]["RaceTable"]["Races"][0][
            "QualifyingResults"
        ][0]["Q3"]
        dicMON[f"{i}"] = qualiDataMON
        qualiDataItaly = response_dataItaly["MRData"]["RaceTable"]["Races"][0][
            "QualifyingResults"
        ][0]["Q3"]
    ...
    f1_pct_change = f1_seconds.drop(
        ["Australia Seconds", "Monoco Seconds", "Monza Seconds"], axis=1
    ).copy()
...
def tech_imp_scatterPlot(f1_quali):
    figMonoco = px.scatter(
        f1_quali,
        x="year",
        y="Monoco Seconds",
        trendline="lowess",
        title="Monoco Grand Prix - High Down Force Track - Qualifying times",
    )
    # figMonoco.show()
    figAus = px.scatter(
        f1_quali,
        ...
        y="Australia Seconds",
        trendline="lowess",
        title="Australian Grand Prix - High Speed/High Down Force Track Qualifying times",
    ...
    figMonz = px.scatter(
        ...
        y="Monza Seconds",
        trendline="lowess",
        title="Italian Grand Prix - High Speed Track Qualifying times",
    )
    # figMonz.show()
...
def tech_pct_scatterPlot(f1_pct_change):
    fig_pct_changeAus = px.scatter(
        f1_pct_change,
        x="year",
        y="mean",
        trendline="lowess",
        title="Average Percentage Change in track time",
    )  # , color='country')
